...
【功能說明】
------------------------------------------------------------
本檔案為 AI_Trading_Journal 量化回測框架的主入口，負責初始化環境、調用回測主流程、協調數據載入、統計分析、用戶互動、回測執行、結果導出等。
...
from core.backtest_engine.Base_backtester import BaseBacktester
from core.performance_metrics.Base_metricstracker import BaseMetricTracker
from core.analytics.AutocorrelationTest_statanalyser import AutocorrelationTest
from core.analytics.Base_statanalyser import BaseStatAnalyser
from core.analytics.CorrelationTest_statanalyser import CorrelationTest
from core.analytics.DistributionTest_statanalyser import DistributionTest
from core.analytics.ReportGenerator_statanalyser import ReportGenerator
from core.analytics.SeasonalAnalysis_statanalyser import SeasonalAnalysis
from core.analytics.StationarityTest_statanalyser import StationarityTest
...
# Assets should be in core/assets if they exist, or we can create them. 
# For now, let's point to core/visualization/assets if that's where plotter looks, or just project_root/assets
# Based on directory list, there is no assets folder in core.
# But let's set it to something reasonable.
os.environ["DASH_ASSETS_FOLDER"] = os.path.join(os.path.dirname(__file__), "visualization", "assets")
listener = None
log_queue = None
...
def setup_logging(log_queue=None):
    ...
    if multiprocessing.current_process().name == "MainProcess":
        ...
        listener.start()
        root_logger = logging.getLogger("AI_Trading_Journal")
...
def main():
    ...
    logger = logging.getLogger("AI_Trading_Journal")

    logger.info("程式開始執行")

    console.print(
        Panel(
            "[bold #dbac30]🚀 AI_Trading_Journal[/bold #dbac30]\n[white]The best backtest engine for non-coders and quant beginners (probably).[/white]\n\n"
            "🌐 Github: https://github.com/AI_Trading_Journal/AI_Trading_Journal\n"
            "🌍 Website: https://AI_Trading_Journal.com\n"
            "💎 Quant Lifetime Membership: https://AI_Trading_Journal.com/membership\n"
            "💬 Discord: \n"
            "✈️ Telegram: https://t.me/AI_Trading_Journalgroup",
    ...
    try:
        if choice == "1":
            ...
            from core.data_feed.base_loader import BaseDataLoader
            ...
            if hasattr(data_loader, 'skip_statanalyser') and data_loader.skip_statanalyser:
                ...
                if run_plotter == "y":
                    try:
                        from core.visualization.Base_plotter import BasePlotter

                        plotter = BasePlotter(logger=logger)
                        core.visualization.run(host="127.0.0.1", port=8050, debug=False)
                    except Exception as e:
                        print(f"❌ 可視化平台啟動失敗: {e}")
                return
            else:
            ...
            try:
                # 導入 autorunner 模組
                from core.automation.Base_autorunner import BaseAutorunner
