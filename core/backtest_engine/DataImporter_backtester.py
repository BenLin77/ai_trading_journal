"""
DataImporter_core.backtest_engine.py

【功能說明】
------------------------------------------------------------
本模組為 AI_Trading_Journal 回測框架的數據導入工具，負責從外部來源載入回測所需的行情數據，支援多種格式與來源，並確保數據結構與回測引擎相容。
...
flowchart TD
    ...
    B -->|載入數據| C[core.data_feed.base_loader]
    C -->|原始數據| D[標準化處理]
...
- 依賴 core.data_feed.base_loader 進行實際數據載入
...
- dataloader 模組文檔
- Base_core.backtest_engine.py、BacktestEngine_core.backtest_engine.py
- 專案 README
"""
...
try:
    from core.data_feed.base_loader import DataLoader
except ImportError as e:
    logging.error(f"無法導入 DataLoader: {str(e)}")
    raise ImportError("請確認 core.data_feed.base_loader 模組存在並可導入。")
