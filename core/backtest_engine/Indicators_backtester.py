"""
Indicators_core.backtest_engine.py

【功能說明】
------------------------------------------------------------
本模組為 AI_Trading_Journal 回測框架的「指標協調與信號產生器」，統一管理所有技術指標的註冊、調用、參數組合產生與信號產生接口，支援多種指標型態與細分策略。
- 提供統一的指標註冊與管理機制
...
class IndicatorsBacktester:
    ...
    def _build_indicator_alias_map(self) -> Dict[str, Tuple[str, int]]:  # pylint: disable=too-complex
        ...
        try:
            module = importlib.import_module(
                "core.backtest_engine.MovingAverage_Indicator_backtester"
            )
        ...
        try:
            module = importlib.import_module(
                "core.backtest_engine.BollingerBand_Indicator_backtester"
            )
        ...
        # HL
        try:
            module = importlib.import_module("core.backtest_engine.HL_Indicator_backtester")
        ...
        try:
            module = importlib.import_module(
                "core.backtest_engine.Percentile_Indicator_backtester"
            )
        ...
        # VALUE
        try:
            module = importlib.import_module("core.backtest_engine.VALUE_Indicator_backtester")
        ...
        # TA (added)
        try:
            module = importlib.import_module("core.backtest_engine.TA_Indicator_backtester")
            if hasattr(module, "TAIndicator") and hasattr(
                module.TAIndicator, "STRATEGY_DESCRIPTIONS"
            ):
                for i, desc in enumerate(
                    module.TAIndicator.STRATEGY_DESCRIPTIONS, 1
                ):
                    if i <= 8:
                        alias_map[f"TA{i}"] = ("TA", i)
        except Exception as e:
            self.logger.warning(f"無法獲取TA指標描述: {e}")
    ...
    def get_indicator_params(  # pylint: disable=unused-argument
        ...
        if alias:
            ...
            module = importlib.import_module(f"core.backtest_engine.{module_name}")
    ...
    def get_available_indicators(self) -> List[str]:  # pylint: disable=too-complex
        ...
        try:
            module = importlib.import_module(
                "core.backtest_engine.TA_Indicator_backtester"
            )
            if hasattr(module, "TAIndicator") and hasattr(
                module.TAIndicator, "STRATEGY_DESCRIPTIONS"
            ):
    ...
    def calculate_signals(  # pylint: disable=unused-argument
        ...
        elif indicator_type == "TA":
            signals = self._calculate_ta_signals(data, params, predictor)
        else:
            raise ValueError(f"不支援的指標類型: {indicator_type}")
    ...
    def _calculate_ta_signals(
        ...
        try:
            ...
            indicator_cls = getattr(module, "TAIndicator")
            indicator = indicator_cls(data, params, logger=self.logger)
            signals = indicator.generate_signals(predictor)
            return signals
        except Exception:
