"""
Base_core.backtest_engine.py

【功能說明】
------------------------------------------------------------
本模組為 AI_Trading_Journal 回測框架的「回測流程協調器」，負責協調數據載入、用戶互動、回測執行、結果導出等全流程。
...
class BaseBacktester:
    ...
    def _display_available_indicators(self) -> str:  # pylint: disable=too-complex
        ...
        try:
            module = __import__(
                "core.backtest_engine.HL_Indicator_backtester", fromlist=["HLIndicator"]
            )
        ...
        try:
            module = __import__(
                "core.backtest_engine.VALUE_Indicator_backtester", fromlist=["VALUEIndicator"]
            )
